"""
dashboard/server.py — Aethelark Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "Aethelark Uploads",
        Path.home() / "Documents" / "Aethelark Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'Aethelark-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"Aethelark Dashboard Port {port}"
        prog_rule  = "Aethelark Dashboard Python"
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until Aethelark is run as Administrator.")
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in Aethelark to get a new QR code.</p>
</div></body></html>""")
...
<p>Connecting to Aethelark…</p>
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in Aethelark UI to get the QR code.")
        try:
            await uvicorn.Server(cfg).serve()
        except OSError as e:
            print(f"[Dashboard] Port in use, disabled: {e}")
