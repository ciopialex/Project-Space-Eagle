...
def _linux_audio(kind: str, arg: int | None = None) -> bool:
    ...
    for b in backends:
        try:
            if run_cmd(b[kind], capture_output=True, timeout=5).returncode == 0:
                return True
        except Exception:
            continue
...
def volume_set(value: int):
    ...
    if _OS == "Windows":
        ...
        run_cmd(["osascript", "-e", f"set volume output volume {value}"],
            capture_output=True)
        return
    else:
...
def brightness_up():
    if _OS == "Darwin":
        ...
        if shutil.which("brightnessctl"):
            run_cmd(["brightnessctl", "set", "+10%"], capture_output=True)
...
def maximize_window():
    if _OS == "Darwin":
        ...
        try:
            run_cmd(["wmctrl", "-r", ":ACTIVE:", "-b", "add,maximized_vert,maximized_horz"],
                capture_output=True)
        except Exception:
...
def snap_left():
    if _OS == "Windows":
        ...
        try:
            run_cmd(["wmctrl", "-r", ":ACTIVE:", "-e", "0,0,0,960,1080"],
                capture_output=True)
        except Exception as _e:
...
def snap_right():
    if _OS == "Windows":
        ...
        try:
            run_cmd(["wmctrl", "-r", ":ACTIVE:", "-e", "0,960,0,960,1080"],
                capture_output=True)
        except Exception as _e:
