...
(function () {
  ...
  var SHELL =
    ...
    +     '<button class="set-mini" id="set-wa-btn">Link</button></div>'
    ...
    +     '<div class="t">YouTube <span class="set-badge off" id="set-yt-badge">CHECKING</span></div>'
    +     '<div class="d" id="set-yt-d">Liked videos, playlists and subscriptions.</div></div>'
    +     '<button class="set-mini" id="set-yt-btn">Connect</button></div></div>'
  ...
  window.aethelark.setSettings = function (s) {
    ...
    var yt = (s.accounts || {}).youtube || {};
    var ytb = document.getElementById('set-yt-badge'),
        ytd = document.getElementById('set-yt-d'),
        ytn = document.getElementById('set-yt-btn');
    if (ytb) {
      ytb.textContent = (yt.label || 'Checking').toUpperCase();
      ytb.className = (yt.state === 'ready') ? 'set-badge' : 'set-badge off';
      // The ACTION, not the diagnosis. Each state has a different fix and the
      // user should never have to work out which wall they are at.
      ytd.textContent = yt.action || 'Liked videos, playlists and subscriptions.';
      ytn.style.display = (yt.state === 'ready') ? 'none' : '';
      ytn.textContent = (yt.state === 'needs_api') ? 'Turn on'
                      : (yt.state === 'needs_scope') ? 'Reconnect' : 'Connect';
    }
  ...
  function wire() {
    ...
    var ytBtn = document.getElementById('set-yt-btn');
    if (ytBtn) ytBtn.onclick = function () {
      this.textContent = 'Working…';
      if (pb() && pb().connect_youtube) pb().connect_youtube();
    };
